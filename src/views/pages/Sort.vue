	...
	<div class="new">
		<div class="selector mt20 main-wdith">
			...
			<div v-if="hasShow">
				...
				<Page :total="totalSize" size="small" show-elevator class="page" :page-size='this.pageSize' :current='this.page' @on-change="handlePage"></Page>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getParams() {
				//首页查看更多（直接通过关键字查找）（从其他页面进入或nav中的搜索框进入重置）
				this.resetAttribute();
			...
			getTop() {
				...
				this.$axios({
					...
					for(let index = 0; index < this.type.length; index++) {
						if(this.type[index].id == this.$route.query.catalog) {
							this.getList('type', this.$route.query.catalog, index)
						} else {
